...
let defaultTemplate = index_1.workspace.configManager.config.runtime.template;
let defaultParent = index_1.workspace.configManager.config.runtime.parent;
let defaultFolder = index_1.workspace.configManager.config.runtime.folder;
let defaultEntrypoint = index_1.workspace.configManager.config.runtime.entrypoint;
...
program
    ...
    utils_1.run(() => __awaiter(this, void 0, void 0, function* () {
        if (!runtimeVersion) {
            runtimeVersion = index_1.workspace.runtimes.getCurrentVersion(name, companyDomain);
        }
        logger.info(`Unregistering from stamp ${stamp} version ${runtimeVersion} of runtime ${name} from ${companyDomain}`);
        yield index_1.workspace.runtimes.unregister(name, companyDomain, runtimeVersion, stamp);
